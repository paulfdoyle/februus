...
void 	get_subrect_dim (char *keyval, int *dimensions);
...
// Since we are not opening the files at the same time we don't need this code.
	// If no files found end the program as there is nothing to process
//	if (count <= 0) {bail("No files in this directory\n"); }

	// Check how many files we can open
...
//    }
    ...
    for (i=0;i<count;++i) {
			...
			if (( anaxes[0] != cnaxes[0] || anaxes[1] != cnaxes[1] )) {
				subrect=1;
				// create the new empty output file which will be an empty copy of the original datafile
				iskey[0] = '\0';  // ensure string is empty

				//read and verify that if there is a subrect keyword
			//	printf ("4a\n");

				ffgky(datafptr, TSTRING, "SUBRECT",iskey , NULL ,&status);
				if (status) {
					if (status == 202)
						 bail("Data image different from Bias/Flat in size and no SUBRECT Keyword");
					else {
						 fits_report_error(stderr, status); // print error message
						 return(status);
					}
					 ...
					 get_subrect_dim(iskey,subrectdim);
					 //printf ("x1 = %d, x2 = %d, y2 = %d, y1= %d\n",subrectdim[0],subrectdim[1],subrectdim[2],subrectdim[3]);
					 indexpix[0] = subrectdim[0];  // subrect shows us the first pixel to use, subtract 1 to correct for
													 // C arrays starting at 0 and not position 1
				}
			}


			// Here we process a single image
		...
		//		printf ("read row %ld  pixels read = %ld first val = %lf\n",firstpix[1],ndpixels,apix[0]);

				if (subrect) 	
                                        {
					//printf ("calculate index into bias/flat file");
							// Only need to index into image with a subrect exists
					indexpix[1] = firstpix[1]+subrectdim[3]-1;  // index into the correct row of subrect
				}
				else
				...
				//printf ("read FLAT row %ld  pixels read = %ld first val = %lf\n",indexpix[1],ndpixels,bpix[0]);
				if (fits_read_pix(mbfptr, TDOUBLE, indexpix, ndpixels, NULL, cpix, NULL, &status)) {
							  bail("Failed to read Bias File row %ld \n",indexpix[1]);
				}

				//printf ("read BIAS row %ld  pixels read = %ld first val = %lf\n",indexpix[1],ndpixels,cpix[0]);

				// Tmp code for testing second image
				// for ( firstpix[2] = 2;
...
void get_subrect_dim (char *keyvalue, int *dim)
{
		sscanf(keyvalue, "%d, %d, %d, %d", &dim[0],&dim[1],&dim[2],&dim[3]);
